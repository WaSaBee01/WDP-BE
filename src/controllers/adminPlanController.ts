...
import { Meal, Exercise, Plan } from '../models';

interface PlanMealInput { time: string; meal: string }
interface PlanExerciseInput { time: string; exercise: string }
...
export const getAllPlans = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to get plans' });
  }
};
