...
export const getProgressEntries = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to get progress entries' });
  }
};

// Get single progress entry by date
export const getProgressEntryByDate = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized', message: 'User not authenticated' });
    }
    ...
    if (!entry) {
      return res.status(404).json({ error: 'Not found', message: 'Progress entry not found' });
    }

    return res.status(200).json({ success: true, data: entry });
  } catch {
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to get progress entry' });
  }
};
...
export const upsertProgressEntry = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to save progress entry' });
  }
};
...
export const applyDailyPlan = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!planId || !startDate || !endDate) {
      return res.status(400).json({ error: 'Validation error', message: 'Plan ID, start date and end date are required' });
    }

    const plan = await Plan.findById(planId).populate('meals.meal').populate('exercises.exercise');
    if (!plan) {
    ...
    while (currentDate <= end) {
      ...
      const meals = plan.meals.map((m: { time: string; meal: { _id?: unknown } | unknown }) => ({
        time: m.time,
        mealId: (m.meal as { _id?: unknown })?._id || m.meal,
        completed: false,
      }));

      const exercises = plan.exercises.map((e: { time: string; exercise: { _id?: unknown } | unknown }) => ({
        time: e.time,
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to apply daily plan' });
  }
};
...
export const applyWeeklyPlan = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!weeklyPlanId || !weekStartDate) {
      return res.status(400).json({ error: 'Validation error', message: 'Weekly plan ID and week start date are required' });
    }

    const weeklyPlan = await WeeklyPlan.findById(weeklyPlanId);
    if (!weeklyPlan) {
      return res.status(404).json({ error: 'Not found', message: 'Weekly plan not found' });
    }

    // Parse date and normalize to UTC to avoid timezone issues
    const inputDate = new Date(weekStartDate + 'T00:00:00.000Z');
    
    // Ensure the date is a Monday (day 1 in ISO week, where Monday = 1)
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (dayPlanId) {
        const dayPlan = await Plan.findById(dayPlanId).populate('meals.meal').populate('exercises.exercise');
        if (dayPlan) {
          const meals = dayPlan.meals.map((m: { time: string; meal: { _id?: unknown } | unknown }) => ({
            time: m.time,
            mealId: (m.meal as { _id?: unknown })?._id || m.meal,
            completed: false,
          }));

          const exercises = dayPlan.exercises.map((e: { time: string; exercise: { _id?: unknown } | unknown }) => ({
            time: e.time,
            exerciseId: (e.exercise as { _id?: unknown })?._id || e.exercise,
            completed: false,
          }));
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to apply weekly plan' });
  }
};
...
export const toggleCompletion = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!date || type === undefined || index === undefined) {
      return res.status(400).json({ error: 'Validation error', message: 'Date, type and index are required' });
    }
    ...
    if (!entry) {
      return res.status(404).json({ error: 'Not found', message: 'Progress entry not found. Please add meals/exercises for this day first.' });
    }

    if (type === 'meal') {
      if (!entry.meals || !entry.meals[index]) {
        return res.status(400).json({ error: 'Validation error', message: `Meal at index ${index} not found` });
      }
      entry.meals[index].completed = !entry.meals[index].completed;
    } else {
      if (!entry.exercises || !entry.exercises[index]) {
        return res.status(400).json({ error: 'Validation error', message: `Exercise at index ${index} not found` });
      }
...
export const deleteProgressEntry = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Internal server error', message: 'Failed to delete progress entry' });
  }
};
